...
# --- lncRNA similarity
def get_index(lncrna):  # 获得lncrna和疾病有关联的的下标
    index = []
...
def get_sim_all(l1_diseases, l2_diseases, disease_sim):
    ...
    for i in range(len(l1_diseases)):
        dis_sim_temp = get_sim(l1_diseases[i], l2_diseases, disease_sim)  # l1disease[i] 和 l2disease【所有】 的联系值
        diss_sims1.append(dis_sim_temp)  # 取最大的联系值
    l1_dis = np.sum(diss_sims1)  # 求l1关联疾病的 与 l2关联疾病 的最大联系值 的 【和】记为l1dis

    diss_sims2 = []
    for i in range(len(l2_diseases)):
        dis_sim_temp = get_sim(l2_diseases[i], l1_diseases, disease_sim)  # l2disease[i]和l1disease【所有】的联系值
        diss_sims2.append(dis_sim_temp)  # 取最大的联系值
    l2_dis = np.sum(diss_sims2)  # 取l2关联疾病的与l1关联疾病的最大联系值的【和】记为l2dis

    l_dis_sim = np.sum((l1_dis, l2_dis))  # 求总和
    return l_dis_sim


def get_sim_value(lncrna1, lncrna2, disease_sim):  # ln1 = A[0]

    lnc1_dis_index = get_index(lncrna1)  # 得到lncrna1的下标有1的有关联的disease 是一个数组
    lnc2_dis_index = get_index(lncrna2)  # 得到lncrna2的下标有1的有关联的disease

    lnc1_dis_len = len(lnc1_dis_index)  # 计算lncrna1的有关联的disease个数
    lnc2_dis_len = len(lnc2_dis_index)  # 计算lncrna2的有关联的disease个数
    length_l1_l2 = lnc1_dis_len + lnc2_dis_len  # 计算rna12所有的长度有关联的disease
    ...
    return sim  # 求的为l1和l2的相似性


# 求所有licrna之间的联系
def get_all_sim1(lnc_dis, dis_sim):  # lncdis即为所有的lncrna
    lnc_length = len(lnc_dis)  # 计算lncrna的长度
    lnc_sim = np.zeros((lnc_length, lnc_length))  # 初始化lncsim设lncrna之间的关系都为0
    # print(np.shape(lnc_sim))

    for i in range(lnc_length):
        for j in range(lnc_length):
            sim = get_sim_value(lnc_dis[i], lnc_dis[j], dis_sim)  # 得到lnc[i]和lnc[j]之间的相似值
            lnc_sim[i, j] = sim  # 将计算的值带入lncsim里面
    for i in range(lnc_length):
        lnc_sim[i, i] = 1  # licrna相同的 联系自然为1
    return lnc_sim
